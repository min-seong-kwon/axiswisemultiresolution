...
def split_until_thres(awmrblock, 
    ...
    vunit_ori = volume_units_16[unit_index]
    ori_shape = np.array(vunit_ori.D.shape)
    pad_tsdf = get_padded(vunit_ori, unit_index, volume_units_16)
    ...
    block = get_AWMR_block(volume_units_16, 
                        volume_units_8, 
                        volume_units_4,
                        axisres, 
                        unit_index,
    ...
    pad = pad_awmr_from_original(block,
                                volume_units_16,
                                volume_units_8,
                                volume_units_4,
                                axisres, 
    ...
    if len(np.asarray(currentmesh.vertices)):
        if version==1:
            currenterror = customChamfer(mesh, currentmesh)
        else:
            currenterror = custom_ChamferDistance(mesh,currentmesh)
    else:
        currenterror = np.inf
    ...
    for ax in [np.array([1,0,0]), np.array([0,1,0]), np.array([0,0,1])]:
        ...
        if max(newaxisres) <= max_res:
            ...
            block1 = get_AWMR_block(volume_units_16, 
                                    volume_units_8, 
                                    volume_units_4,
                                    newaxisres, 
                                    unit_index,
                                    start_point1,
                                    option=sample_option)
            block2 = get_AWMR_block(volume_units_16, 
                                    volume_units_8,
                                    volume_units_4,
                                    newaxisres, 
                                    unit_index,
            ...
            if len(np.asarray(newmesh.vertices)):
                if version == 1:
                    error = customChamfer(mesh, newmesh)
                else:
                    error = custom_ChamferDistance(mesh,newmesh)
            else:
                error = np.inf
    ...
    tsdf1 = get_AWMR_block(volume_units_16, 
                        volume_units_8, 
                        volume_units_4,
                        newaxisres, 
                        unit_index, 
    ...
    tsdf2 = get_AWMR_block(volume_units_16, 
                        volume_units_8, 
                        volume_units_4,
                        newaxisres, 
                        unit_index, 
    ...
    if minerror>thres:
        if awmrblock.left is not None:
            split_until_thres(awmrblock=awmrblock.left, 
                                thres=thres,
                                volume_units_16=volume_units_16,
                                volume_units_8=volume_units_8,
                                volume_units_4=volume_units_4,
                                axisres=newaxisres, 
...
def split_until_thres_octree(awmrblock,
                            thres,
                            volume_units_16,
                            volume_units_8,
                            volume_units_4,
                            ...
                            version=1,
                            sample_option='pool'):
#    debug_list = [(0, 0, 0), (0, 0.25, 0), (0.5, 0, 0), (0.5 , 0.25, 0)]
#    if (tuple(start_point)==(0,0,0)) and (tuple(axisres)==(16,16,16)):
#        print(tuple(axisres), tuple(axisres)==(16,32,16))
    version=1
    if for_train and (awmrblock.tsdf.M is None):
        pad_for_mask = pad_awmr_from_original(awmrblock.tsdf.D,
                                          volume_units_16,
    ...
    if np.all(axisres == axisres[0]): # 축별 해상도가 같을 때 
        ...
        # if len(np.asarray(mesh.vertices))==0: # if len(np.asarray(mesh.vertices))==0 and np.all(axisres == axisres[0]):
        #     return
        # if len(np.asarray(currentmesh.vertices))==0: 
        #     currenterror = np.inf
        
        if  len(np.asarray(mesh.vertices))==0 or \
            len(np.asarray(currentmesh.vertices))==0:
            return
        ...
        if max(newaxisres) <= max_res:
            ...
            if  len(np.asarray(mesh.vertices))==0 or \
                ...
                if version==1:
                    ...
                    error = custom_ChamferDistance(mesh, newmesh)
            # if len(np.asarray(newmesh.vertices)):
            #     # error = igl.hausdorff(np.asarray(mesh.vertices),
            #     #                     np.asarray(mesh.triangles),
            #     #                     np.asarray(newmesh.vertices),
            #     #                     np.asarray(newmesh.triangles))
            #     try:
            #         error = customChamfer(mesh, newmesh)
            #     except:
            #         print("Error !", np.asarray(mesh.vertices).shape)
            #         error = igl.hausdorff(np.asarray(mesh.vertices),
            #                             np.asarray(mesh.triangles),
            #                             np.asarray(newmesh.vertices),
            #                             np.asarray(newmesh.triangles))
    ...
    if (minerror>thres) or not np.all(newaxisres == newaxisres[0]):
        if awmrblock.left is not None:
            split_until_thres_octree(awmrblock=awmrblock.left, 
                                thres=thres,
                                volume_units_16=volume_units_16,
                                volume_units_8=volume_units_8,
                                volume_units_4=volume_units_4,
                                ...
                                sample_option=sample_option)
        if awmrblock.right is not None:
            split_until_thres_octree(awmrblock=awmrblock.right, 
                                thres=thres,
                                volume_units_16=volume_units_16,
                                volume_units_8=volume_units_8,
                                volume_units_4=volume_units_4,
...
def pad_awmr_from_original(tsdf,
    ...
    for i in range(7):
        ...
        if (sample_from in volume_units_16.keys()) or (sample_from in volume_units_8.keys()) or (sample_from in volume_units_4.keys()):
            ...
            temp_arr = get_AWMR_block(volume_units_16, volume_units_8, volume_units_4, axisres, sample_from, new_start_point)
