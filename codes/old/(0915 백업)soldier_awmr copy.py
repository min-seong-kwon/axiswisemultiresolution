...
target_path = fr'../results/[TSDF]{dataset_name}/AWMR/voxsize_{finest_voxel_size:.1f}'
mesh_filename = target_path + fr"/{dataset_name}_awmr_thres={thres}.ply"
txt_filename = target_path + fr"/{dataset_name}_awmr_thres={thres}.txt"
...
# # # p1 = np.array([204.950607, 1006.562744, 165.825638])
# # # p2 = np.array([333.530640, 804.470276, 292.692474])
...
for k in tqdm(volume_units_32.keys()):
    ...
    pad_finest = pad_3d(volume_units_32[k].D, k, volume_units_32)
    
    if (np.min(pad_tsdf)*np.max(pad_tsdf) >= 0) and (np.min(pad_finest)*np.max(pad_finest) >= 0):
        continue
